...
string html = await http.GetStringAsync(@"https://www.lensrentals.com/blog/2019/04/just-mtf-charts-sony-fe-mount-prime-lenses/");
HtmlParser parser = new();
IHtmlDocument doc = await parser.ParseDocumentAsync(html);
string[] imageUrls = doc.QuerySelectorAll("img[width=600]")
	.Select(x => x.GetAttribute("src"))
	.ToArray();
Console.WriteLine($"Find {imageUrls.Length} pictures...");

Util.SetPassword("dotnet2021-cv-zhoujie-demo1", Util.CurrentQuery.Location);
Environment.CurrentDirectory = Util.GetPassword("dotnet2021-cv-zhoujie-demo1");
Directory.CreateDirectory("./resources");
File.WriteAllText("./resources/.gitignore", "*.png");
await Parallel.ForEachAsync(imageUrls, QueryCancelToken, async (url, ct) =>
{
	string targetPath = "./resources/" + Path.GetFileName(url);
